...
######### 3b. An example of replacing the breed variable with another variable, here we use Purebred in the model 
...
# Tabulate the  results of the model with no interactions 
...
#get the area under the roc curve for this model 

pROC_obj <- roc(OS$OSf, multi_model$fitted.values, plot=TRUE, legacy.axes=TRUE, print.auc=TRUE)

pROC_obj$auc

#get the lrtest to get an overall p-value for the purebred variable by comparing to a null model without a purebred variable 
nopure_model <- glm(OSf ~ agef + Sex_Neuterf, data = OS, family = binomial(link="logit"))
lrtest(multi_model, nopure_model)

#Now run the model including pairwise interactions between purebred and age and see if the interaction is significant
...
##If interactions are significant, run the model including pairwise interactions between all terms 
...
#Repeat this steps using any other variable in place of breed in the model 